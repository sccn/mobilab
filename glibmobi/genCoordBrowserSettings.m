function varargout = genCoordBrowserSettings(varargin)
...
%      singleton*.
%
%      H = GENCOORDBROWSERSETTINGS returns the handle to a new GENCOORDBROWSERSETTINGS or the handle to
%      the existing singleton*.
%
%      GENCOORDBROWSERSETTINGS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GENCOORDBROWSERSETTINGS.M with the given input arguments.
%
%      GENCOORDBROWSERSETTINGS('Property','Value',...) creates a new GENCOORDBROWSERSETTINGS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before genCoordBrowserSettings_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to genCoordBrowserSettings_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help genCoordBrowserSettings

% Last Modified by GUIDE v2.5 16-Sep-2011 17:16:56

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @genCoordBrowserSettings_OpeningFcn, ...
    'gui_OutputFcn',  @genCoordBrowserSettings_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before genCoordBrowserSettings is made visible.
function genCoordBrowserSettings_OpeningFcn(hObject, eventdata, handles, varargin)
...
set(handles.checkbox6,'Value',~browserObj.coordinates(1));
set(handles.checkbox7,'Value',~browserObj.coordinates(2));
set(handles.checkbox8,'Value',~browserObj.coordinates(3));
...
function varargout = genCoordBrowserSettings_OutputFcn(hObject, eventdata, handles)
...
try
    ...
    elseif tmp(end) > browserObj.streamHandle.numberOfChannels;
        error('Cannot display one of these channels.');
    elseif tmp(1) < 1;
        error('Cannot display one of these channels.');
    else
    ...
    browserObj.nowCursor = browserObj.streamHandle.timeStamp(browserObj.timeIndex(1)) + userData.windowWidth/2;
    ...
    userData.coordinates(1)    = get(handles.checkbox6,'Value');
    userData.coordinates(2)    = get(handles.checkbox7,'Value');
    userData.coordinates(3)    = get(handles.checkbox8,'Value');
