...
[mstart,junk]=size(start); [mstop,~]=size(stop); [mcrossdir,junk]=size(crossdir);
...
%	% make or modify the arrows
	for k=1:narrows,
		...
		if newpatch(k)||newline(k),
			if newpatch(k),
				H(k) = patch('XData',xyz{2},'YData',xyz{4},'ZData',xyz{6},'Tag','Arrow','Parent',ax(1));
			else
				H(k) = line('XData',xyz{2},'YData',xyz{4},'ZData',xyz{6},'Tag','Arrow','Parent',ax(1));
			end;
