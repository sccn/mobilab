classdef ribbonBrowserHandle < browserHandle
    properties        
        gObjHandle
        windowWidth
        textHandle
    ...
    methods
        %% constructor
        function obj = ribbonBrowserHandle(dStreamObj,defaults)
            if nargin < 2,
            ...
            obj.addlistener('channelIndex','PostSet',@ribbonBrowserHandle.updateChannelDependencies);
            obj.addlistener('font','PostSet',@ribbonBrowserHandle.updateFont);
        ...
        function plotThisTimeStamp(obj,nowCursor)
            ...
            X = [x(1:end-1,1)';x(2:end,1)';x(2:end,2)';x(1:end-1,2)' ];
            Y = [y(1:end-1,1)';y(2:end,1)';y(2:end,2)';y(1:end-1,2)' ];
            t = [time1(1:end-1);time1(2:end);time1(2:end);time1(1:end-1)];
            ...
            plot3(time1,x(:,1),y(:,1),'linewidth',3)
            plot3(time1,x(:,2),y(:,2),'r','linewidth',3);
    ...
    methods(Static)
        function updateChannelDependencies(~,evnt)
            ...
            evnt.AffectedObject.ribbonCData = -diff(sqrt(x.^2 + y.^2),[],2);
