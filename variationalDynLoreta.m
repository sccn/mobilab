function [J,alpha,beta,T,lambda2,FVU,aic] = variationalDynLoreta(Ut,Y,s2,iLV,L,alpha,beta,options)
...
alpha_old = alpha;
beta_old = beta;
aic_hist = zeros(options.maxIter-1,1);
for it=1:options.maxIter
    ...
    % computing AIC
    %aic = -2*log(sigma2) + 2*gamma;
    aic = -2*log(sigma2) + 2*gamma + 2*gamma*(gamma+1)/(p-gamma-1); % AICc
    
%     % if the new model is worst than the one before, exit
%     if aic > aic_old,
%         alpha = alpha_old;
%         beta = beta_old;
%         break;
%     end
    ...
    % to prevent oscillations around the same model
    if it > options.maxIter/4 && std(abs(diff(aic_hist(it-6:it-1)))) < options.maxTol,
        alpha = alpha_old;
        beta = beta_old;
        break;
    end   
...
FVU = 1-sum(var(H*Y))/sum(var(Y));
if exist('lambda2Out','var'), lambda2 = lambda2Out;end
end
