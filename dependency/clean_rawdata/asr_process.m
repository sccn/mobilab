...
%          The data should be high-pass filtered (the same way as for asr_calibrate).
%   
%   SamplingRate : sampling rate of the data in Hz (e.g., 250.0)
%
%   State : initial filter state (determined by asr_calibrate)
%
...
%               value is WindowLength/2. Default: 0.125
%
...
%               default: min(5000,1/2 * free memory in MB)
%
    ...
    if ~isempty(range)
        ...
        for j=1:length(update_at)
            ...
            if ~trivial || ~last_trivial                
                ...
                data(:,subrange) = bsxfun(@times,blend,R*data(:,subrange)) + bsxfun(@times,1-blend,last_R*data(:,subrange));
            end
