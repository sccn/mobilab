...
%                'EqualizeScaling' : optionally equalize the channel scaling
%
% Examples:
...
opts = hlp_varargin2struct(varargin, ...
    {'yrange','YRange'}, [0.1 0.8], ...         % y range of the figure occupied by the signal plot
    {'yscaling','YScaling'}, 3, ...             % distance of the channel time series from each other in std. deviations
    ...
    {'equalize_scaling','EqualizeScaling'},false);    % optionally equalize the channel scaling
...
lastPos = 0;
hFig = figure('ResizeFcn',@on_window_resized); hold; axis();
hAxis = gca;
hSlider = uicontrol('style','slider'); on_resize();
...
on_update();
    ...
    function repaint(relPos,moved)
        ...
        % title
        title(sprintf('%s - [%.1f - %.1f]', old.filename, new.xmax*relPos, new.xmax*relPos + opts.wndlen));
        ...
        iqrange = mean([iqr(oldwnd');iqr(oldwnd')])';        
