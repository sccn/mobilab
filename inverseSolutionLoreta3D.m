function [J,lambdaOpt,sig] = inverseSolutionLoreta3D(Y,K,L,nlambda,plotGCV,threshold)
...
% sig = inverseSolutionLoreta(Y,sum(K.^2,3),L,nlambda,0,[]);

K = kron( diag([1 0 0]),K(:,:,1)/L) + kron( diag([0 1 0]),K(:,:,2)/L) + kron( diag([0 0 1]),K(:,:,3)/L);

% Zeros = sparse(dim(1),dim(2)/3);
% K = [K(:,:,1) Zeros Zeros; Zeros K(:,:,2) Zeros; Zeros Zeros K(:,:,3)];

L = kron(eye(3),L);
Y = kron([1;1;1],power(3,-0.5)*Y);
...
% eliminating artefacts introduced due to the inversion of the laplacian
% operator
warning off all;
options = statset('Display','off');
gm = gmdistribution.fit(J,2,'Options',options);
cl = gm.cluster(J);
[~,sloc] = sort(gm.Sigma);
if gm.PComponents(sloc(1))/gm.PComponents(sloc(2)) > 10
    outliers = cl==sloc(2);
    p = gm.posterior(J(outliers));
    J(outliers) = p(:,sloc(1)).*J(outliers);
...
J = reshape(J,[dim(2)/3 3]);
Jt = sqrt(sum(J.^2,2));
try %#ok
    th = prctile(Jt,threshold);
