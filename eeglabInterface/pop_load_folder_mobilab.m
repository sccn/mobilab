...
%%
function allDataStreams = pop_load_folder_mobilab(source,mobiDataDirectory)
...
try
    ...
    if dispCommand
        disp('Running:');
        disp(['  allDataStreams = dataSourceFromFolder( ''' source ''' , ''' mobiDataDirectory ''');' ]);
    end
    
    allDataStreams =  dataSourceFromFolder(source,mobiDataDirectory);
